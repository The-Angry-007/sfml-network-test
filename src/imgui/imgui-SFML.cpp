...
bool Init(sf::Window& window, const sf::Vector2f& displaySize, bool loadDefaultFont)
{
	...
	// load mouse cursors
	const auto loadMouseCursor = [](ImGuiMouseCursor imguiCursorType, sf::Cursor::Type sfmlCursorType) { s_currWindowCtx->mouseCursors[imguiCursorType] = sf::Cursor::createFromSystem(sfmlCursorType); };
...
void SetCurrentWindow(const sf::Window& window)
{
	auto found = std::find_if(s_windowContexts.begin(),
		...
		[&](std::unique_ptr<WindowContext>& ctx) { return ctx->window->getNativeHandle() == window.getNativeHandle(); });
...
void ProcessEvent(const sf::Window& window, const sf::Event& event)
{
	...
	if (true)
	{
		...
		{
			if (mouseWheelScrolled->wheel == sf::Mouse::Wheel::Vertical || (mouseWheelScrolled->wheel == sf::Mouse::Wheel::Horizontal && io.KeyShift))
			{
...
void Update(const sf::Vector2i& mousePos, const sf::Vector2f& displaySize, sf::Time dt)
{
	...
	if (s_currWindowCtx->windowHasFocus)
	{
		...
		{
			io.MouseDown[i] = s_currWindowCtx->touchDown[i] || sf::Touch::isDown(i) || s_currWindowCtx->mousePressed[i] || sf::Mouse::isButtonPressed((sf::Mouse::Button)i);
...
void updateMouseCursor(sf::Window& window)
{
	...
	if ((io.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange) == 0)
	{
		...
		else
		{
			...
			const sf::Cursor& c = s_currWindowCtx->mouseCursors[cursor] ? *s_currWindowCtx->mouseCursors[cursor] : *s_currWindowCtx->mouseCursors[ImGuiMouseCursor_Arrow];
